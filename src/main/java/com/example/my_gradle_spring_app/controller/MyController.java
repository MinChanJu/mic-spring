...
public class MyController {
    ...
    public List<ContestScore> getContestScore(@PathVariable Long contestId) {
        ...
        List<User> participants = userService.getUserByContestId(contestId);

        List<ContestScore> contestScores = new ArrayList<>();
        for (User user : participants) {
            List<Submit> solveProblems = new ArrayList<>();
            for (Problem problem : problems) {
                Solve solved = solvedService.getSolvedByUserIdAndProblemId(user.getUserId(), problem.getId());
                if (solved == null) solveProblems.add(new Submit(problem.getId(), (short) 0));
