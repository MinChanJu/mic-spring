...
public class MyController {
  ...
  public MyController(CodeService codeService, DataService dataService, MailService mailService,
  ...
  @PostMapping("/mail")
  public ResponseEntity<ApiResponse<String>> sendMail(@RequestBody ReportDTO reportDTO,
      HttpServletRequest request) {
    Token token = (Token) request.getAttribute("token");

    String requestId = UUID.randomUUID().toString();
    Future<ApiResponse<Void>> future = requestQueueService.addRequest(() -> {
      mailService.sendMail(reportDTO, token);
      return new ApiResponse<>(200, true, "메일 요청 성공", null);
    });
    requestResults.put(requestId, future);
